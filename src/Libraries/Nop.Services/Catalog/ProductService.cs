        ...
        public virtual void AdjustInventory(Product product, int quantityToChange, string attributesXml = "", string message = "")
        {
            ...
            if (product.ManageInventoryMethod == ManageInventoryMethod.ManageStockByAttributes)
            {
                ...
                if (combination != null)
                {
                    ...
                    //quantity change history
                    var combinationMessage = string.Format("{0} {1}", 
                        string.Format(_localizationService.GetResource("Admin.StockQuantityHistory.Messages.Combination"), combination.Id), message);
                    AddStockQuantityHistoryEntry(product, quantityToChange, combination.StockQuantity, message: combinationMessage, combinationId: combination.Id);
