        ...
        public ActionResult ProductAttributeCombinationUpdate(ProductModel.ProductAttributeCombinationModel model)
        {
            ...
            if (product.ManageInventoryMethod == ManageInventoryMethod.ManageStockByAttributes)
            {
                var message = string.Format("{0} {1}",
                    string.Format(_localizationService.GetResource("Admin.StockQuantityHistory.Messages.Combination"), combination.Id),
                    _localizationService.GetResource("Admin.StockQuantityHistory.Messages.Combination.Edit"));
                _productService.AddStockQuantityHistoryEntry(product, combination.StockQuantity - previousSrockQuantity,
                    combination.StockQuantity, message: message, combinationId: combination.Id);
            }
            ...
            if (!warnings.Any())
            {
                ...
                if (product.ManageInventoryMethod == ManageInventoryMethod.ManageStockByAttributes)
                {
                    var message = string.Format("{0} {1}",
                        string.Format(_localizationService.GetResource("Admin.StockQuantityHistory.Messages.Combination"), combination.Id),
                        _localizationService.GetResource("Admin.StockQuantityHistory.Messages.Combination.Edit"));
                    _productService.AddStockQuantityHistoryEntry(product, combination.StockQuantity, combination.StockQuantity, message: message, combinationId: combination.Id);
                }
